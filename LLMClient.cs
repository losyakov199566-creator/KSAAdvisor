...
public class LLMClient
{
        ...
        [EnumeratorCancellation] CancellationToken ct = default)
    {
        ...
        // Цикл tool calling — до 4 раундов
        for (int round = 0; round < 4; round++)
        {
            ...
            var body = JsonSerializer.Serialize(new
            {
                ...
                // Исключает провайдеров OpenRouter без поддержки tool calling —
                // без этого иногда прилетает сырой текст спецтокенов вместо
                // структурированного tool_calls. Не ограничивает выбор
                // провайдера сильнее необходимого, цену почти не повышает.
            ...
            while (true)
            {
                ...
                // Отдаём текстовый контент (вне try/catch — для yield)
