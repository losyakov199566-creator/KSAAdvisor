...
public class GameStateReader
{
    ...
    // Все небесные тела в текущей сессии (не корабли)
    ...
    // ── Статический промт ─────────────────────────────────────────────────
    // CoreInstructions — захардкожены, нельзя сломать файлом.
    // Persona (опционально, persona.txt) — только тон и стиль.
    // Детали орбит — через инструмент get_body_info().
    ...
    public string BuildStaticSystemPrompt()
    {
        ...
        Console.WriteLine(File.Exists(personaPath)
            ? $"[KSAAdvisor] persona source: {personaPath}"
            : "[KSAAdvisor] persona source: DefaultPersona");
    ...
    public string GetBurns()
    {
        try
        {
            ...
            for (int i = 0; i < count; i++)
            {
                ...
                // Пробуем получить dV через рефлексию — имя поля может меняться
                string dvStr = "";
                try
                {
    ...
    // ── Tool: другие аппараты в системе ──────────────────────────────────
    ...
    // ── Tool: создать манёвр циркуляризации ───────────────────────────────
    ...
    public string CreateCircularizationBurn(string at)
    {
        try
        {
            ...
            // dV в CCI для циркуляризации
            var dvCci = OrbitalTransfers.DvCciToCircularize(v.Orbit, burnTime);

            // Конвертируем в VLF (систему отсчёта корабля)
            ...
            // Находим патч орбиты в момент манёвра
    ...
    private string GetSkillLevelInstruction() => _config.UserSkillLevel.ToLowerInvariant() switch
    {
        "beginner" =>
            ...
            "in plain language the first time you use them (e.g. \"apoapsis " +
            "(the highest point of your orbit)\"). Stay concise — one short " +
            "clause of explanation, not a lecture.",

        "expert" =>
            "USER SKILL LEVEL: expert. Skip all explanations of standard " +
            "concepts entirely. Use precise terminology, numbers only, no " +
            "hand-holding.",

        // "experienced" (default) — поведение уже задано в CoreInstructions,
    ...
    // Захардкожено намеренно — эти правила влияют на корректность
    // и безопасность действий, их нельзя случайно сломать редактированием
    // или удалением внешнего файла. persona.txt отвечает только за тон.
    private const string CoreInstructions = """
        ...
        Never substitute real astronomical data for game data — the game
        system may differ from the real solar system (different orbits,
        distances, physical properties).
        If a tool doesn't return a value you need, say "the game does not
        provide this data" — never guess or fall back to real-world values.
        When citing a specific number, make it clear it comes from game
        data (e.g. "per current telemetry, dV is 1200 m/s").
        ...
        TIME FORMATTING
        Tool results give exact decimal days for precision. In your own
        text to the user, never show raw decimal days like "0.28 days" —
        convert to natural units (days/hours/minutes/seconds) instead,
        e.g. "6 hours 43 minutes" or "2 days 14 hours". Each tool result
        already includes a human-readable hint (~Xh Ym) — use that phrasing
        style. When calling warp_time, always use the exact decimal days
        value from the tool result, never a value rounded to whole hours
        or minutes.
        ...
        TIME-SENSITIVE DATA
        Telemetry, burns, and positions become stale after a time warp.
        If the user asks about current state, current time-to-burn, or
        anything that may have changed after a warp_time call earlier in
        this conversation, call the relevant tool again instead of reusing
        old values from earlier in the chat.

        WARPING TIME
        Only call warp_time when the user has explicitly confirmed it —
        either in this message ("warp 5 days", "skip to the window") or
        by agreeing to a warp you proposed in your previous reply ("yes",
        "go ahead", "do it"). If you are merely informing the user about
        a window or a burn time, state the number — do not warp on your
        own initiative without that confirmation.
        """;
}